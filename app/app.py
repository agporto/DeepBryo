...
def filter_border(img_size, outputs, classes, pad, confidence):
    ...
    for num, structure in enumerate(boxes):
        if classes == 0:
            idx = [idx for idx, element in enumerate(structure) if condition(element, img_size, pad, confidence)]
            if idx:
...
def filter_by_occlusion(_model, outputs, classes, strictness):
    ...
    for num, structure in enumerate(masks):
        if classes == 0 or num == (classes - 1) :
            idx = [idx for idx, element in enumerate(structure) if autocondition(encoded_masks[num][idx], element.shape[0], element.shape[1], _model, num, strictness)]
            if idx:
                outputs[0][num] = np.delete(outputs[0][num], idx, 0)
                outputs[1][num] = np.delete(outputs[1][num], idx, 0)
    return outputs
...
def condition(x, img, pad, confidence):
            return x[0] < pad[0] or x[1] < pad[1] or x[2] > (img[1] - pad[2]) or x[3] > (img[0] - pad[3]) or x[4] < confidence

def autocondition(element, height, width, model, class_id, strict):
    bbox = mask_util.toBbox(element)
    area, perimeter, solidity, circularity, xc, yc, d1, d2, angle, hu = mask_stats(element)
    array = np.array([[class_id, area, perimeter, solidity, circularity, min(d1,d2) / max(d1,d2), height, width, angle, xc, yc, 
        int(bbox[0]),int(bbox[1]),int(bbox[2]),int(bbox[3])]], dtype= float)
...
def summarize(predictions, class_id, classes, filename, scale= None): 
    ...
    for num, structure in enumerate(bboxes): 
        if class_id == 0 or num == (class_id -1):
            for idx, element in enumerate(structure):
                area, perimeter, solidity, circularity, xc, yc, d1, d2, angle, hu = mask_stats(masks[num][idx])
                annotation_info = {
                    'image_id': filename,
...
def main():
    ...
    confidence = st.sidebar.slider('Confidence', 0., 1.0, 0.5) 

    st.sidebar.markdown('## Filter objects')
    pad_filter = st.sidebar.checkbox('Distance from image border (in pixels)', value=False)
    if pad_filter:
    ...
    #if auto_filter:
    strictness = st.sidebar.slider('Strictness', 0., 1.0, 0.5) 
    ...
    #random = st.button('Random Image')
    uploaded_img = st.file_uploader("Choose an image...", type=['jpg', 'jpeg', 'png', 'tif', 'bmp'], accept_multiple_files=False)
    #print(uploaded_img)
    #if random:
    #    uploaded_img = 
    if uploaded_img: #is not None:
        ...
        out_filtered = filter_border([height, width], out, classes.index(object_type), padding, confidence)

        if auto_filter:
            out_filtered = filter_by_occlusion(autofilter, out_filtered, classes.index(object_type), strictness)

        out_image = model.show_result(img, out_filtered, font_size = label_font, thickness=bbox_thickness)
            ...
            if canvas_result.json_data is not None:
                ...
                if objects.empty is False:
                    if len(objects.left) == 2:
                        ...
                        length = (((objects.left[0] - objects.left[1])**2 + (objects.top[0] - objects.top[1])**2)**0.5)*ratio
                        st.write(f"Success! Your scale is {length/number} pixels per μm.")
                    else:
                        st.write(f"Please select only two points. Use backward arrow to delete points.")
                else:
                    st.write(f"Please select two points: one at the beggining and one at the end of the scale. Then press the leftmost button to submit to the app.")
        ...
        if table:
            if out_filtered:
                if scale is True and not objects.empty:
                    df = summarize(encode_result, classes.index(object_type), model.CLASSES, uploaded_img.name, scale = length/number)
                    if df is not None:
                        ...
                        st.dataframe(df)
                        csv = convert_df(df)
                        st.download_button("Press to Download", csv, f"{uploaded_img.name[:-4]}.csv", "text/csv", key='download-csv')
                    else:
                        st.write("No objects detected.")
                else:
                    df = summarize(encode_result, classes.index(object_type), model.CLASSES, uploaded_img.name)
                    if df is not None:
                        if filter_id is True and selected_ids is not None:
                            df = df.drop(index=selected_ids)
                        st.write("You have not set the scale yet. The current table reports values in pixels.")
                        st.dataframe(df)
                        csv = convert_df(df)
                        st.download_button("Press to Download", csv, f"{uploaded_img.name[:-4]}.csv", "text/csv", key='download-csv')
                    else:
